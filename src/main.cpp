...
void configurePsramUsage() {
    ...
    Serial.printf("[boot] PSRAM enabled total=%u free=%u\n",
...
bool playRequest(const String& url, const String& label, const String& type, const String& source, String& error) {
    deferredActions->playUrl = url.isEmpty() ? settings->light.primaryColor : url;
    deferredActions->playLabel = label.isEmpty() ? settings->light.secondaryColor : label;
    deferredActions->playType = type.isEmpty() ? String(settings->light.effectIndex) : type;
    if (!source.isEmpty()) {
        deferredActions->playSource = source;
    } else {
        deferredActions->playSource = "manual";
    }
