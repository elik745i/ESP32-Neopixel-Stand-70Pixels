...
function updateAudioUiState() {
  const powerEnabled = Boolean(elements.audioMutedToggle?.checked);
  if (elements.audioMutedNote) {
    elements.audioMutedNote.textContent = powerEnabled
      ? "Light output is enabled. Apply changes to update the active effect immediately."
      : "Light output is disabled. Use Apply to turn the strip back on with the selected effect.";
  }
}
...
function renderRecentPlayback() {
  if (!state.recentPlayback.length) {
    elements.recentPlaybackList.innerHTML = '<div class="firmware-item"><div class="firmware-meta"><div class="firmware-title">No recent light presets</div><div class="firmware-subtitle">Applied colors and effects will appear here for one-click reuse.</div></div></div>';
    return;
  }
  ...
  elements.recentPlaybackList.innerHTML = state.recentPlayback.map((item, index) => `
    <div class="firmware-item">
      <div class="firmware-meta">
        <div class="firmware-title">${escapeHtml(item.type || "Static")}</div>
        <div class="firmware-subtitle">${escapeHtml(item.url)}${item.label ? ` | ${escapeHtml(item.label)}` : ""}</div>
      </div>
...
async function setVolume(volumePercent) {
  ...
  setMessage(`Brightness saved at ${volumePercent}%`);
  await loadStatus();
...
async function copyCurrentUrl() {
  ...
  if (!value) {
    toast("No current output to copy");
    return;
  }
  await navigator.clipboard.writeText(value);
  toast("Copied current output");
}
